...
def decode_sol(factory, sol_encoded):
    ...
    for job in sol_encoded:
        ...
        n_ope = ope_job[job]+1
        durations = []
        for i in range(factory.n_mach):
            if (job,n_ope,i+1) in factory.p:
                durations.append(factory.p[(job,n_ope,i+1)])
            else:
                durations.append(-1)
        slots = [(m, max(solution[m], key=lambda item:item[3])[3]) for m in range(1,factory.n_mach+1) if (job,ope_job[job]+1,m) in factory.p]
        slots += [(m, ts) for m, liste_ope in solution.items() for i,_,_,_,ts in enumerate(liste_ope[:len(liste_ope)-1]) if liste_ope[i+1][2]-ts >= durations[m-1]]
