...
def solve(factory):
    ...
    MAX_TIME = 1200 #Durée de la recherche
    k_max = 2 #Nombre de voisinages considérés dans le VND
    MAX_ITER_WITHOUT_IMP = 10 #Nombre d'itérations sans amélioration avant restart
    ...
    #COmparaison avec un potentiel meilleur décodage greedy
    if interpret_mode == 4:
    ...
    #Retour de la solution osu forme de dictionnaire
...
def transform(factory):
    """
    Extraction des donénes de l'objet factory pour utilisation rapide sous forme de paramètres
    :param factory: instance
    :return: nb de jobs, de machines, d'opérations par job, le total d'opérations et une matrice 3D pour des opérations groupées sur les coûts
    """
...
def interpret(solution, n_jobs, n_machines, n_ope, n_total_ope, durations, factory):
    """
    Macro fonction pour le décodage d'une solution sous forme de chromosome
    :return: une solution complete sous forme de planning adapté pour l'évaluation. Elle nécesite un décodage pour le retour
    """
    if interpret_mode == 0:
...
def interpret_greedy(solution, n_jobs, n_machines, n_ope, n_total_ope, durations):
    """
    Décodage privilégiant le remplissage de trous laissés dans le fil d'une mahcine.
    Min cost tie break.
...
def interpret_min(solution, n_jobs, n_machines, n_ope, n_total_ope, durations):
    """
        Décodage privilégiant la machine induisant le makespan le plus faible.
        Implémentation numpy.
    """
    complete_solution = np.full((n_machines,  int(max(n_ope)*2), 4), -1, dtype=np.int)
    complete_solution[:, :, 2:] = np.zeros((n_machines, int(max(n_ope)*2), 2), dtype=np.int)
...
def interpret_base(solution, n_jobs, n_machines, n_ope, n_total_ope, durations):
    """
    Décodage privilégiant la 1ère machine avec le temps final le plus faible.
    Implémentation numpy.
    """
    complete_solution = np.full((n_machines, int(max(n_ope)*2), 4), -1, dtype=np.int)
    complete_solution[:, :, 2:] = np.zeros((n_machines, int(max(n_ope)*2), 2), dtype=np.int)
...
def interpret_base_exact(solution, n_jobs, n_machines, n_ope, n_total_ope, durations):
    """
    Décodage privilégiant la dernière machine avec le temps final le plus faible.
    Implémentation numpy.
    """
...
def interpret_python(factory, sol_encoded):
    """
    Décodage privilégiant la dernière machine avec le temps final le plus faible.
    Implémentation vanilla plus rapide.
    """
    sol = sol_encoded+1
...
def interpret_python_greedy(factory, sol_encoded):
    ...
    for job in sol:
        n_ope = ope_job[job] + 1 #opération à assigner
        ...
        #EMplacents d'insertion possible
        ...
        #Recherche du meilleure emplacement
...
def evaluation(complete_solution):
    """
    Evaluation selon l'implémentation du décodage (numpy ou dico)
    :param complete_solution: solution sous forme de planning
    :return: makespan
    """
    if interpret_mode <= 3:
...
def decode(complete_solution):
    """
    Restitution en dico si l'interprétation a été faite en format numpy
    :param complete_solution: solution sous forme de planning
    :return: planning en implémentation dico
    """
    if interpret_mode <= 3:
        result = dict()
...
def init_ACO(n_jobs, n_ope, n_machines, n_total_ope, durations, factory, n, n_ants, t_max=3.0, t_min=1.0, rho=0.2, alpha=1, beta=1):
    """
    Initilisation par colonie de fourmi sans phase de recherche locale. Variante MMSA
    l'heuristique minimise l temps final de la machine choisi. On fonctionne sur un principe glouton de base.
    Insertion en bout de ligne uniquement.
    :param n: nombre d'itérations
    :param n_ants: nombre de forumis
    :param t_max: limite supérieure des phéromone
    :param t_min: limite inférieure des phéromone
    :param rho: taux de dégration des phéromones
    :param alpha: poids des phéromones dans la construction (exposant)
    :param beta: poids de l'heuristique dans la construction (exposant)
    :return: meilleure soltuion générée
    """
    best_score = 100000
    best_sol = None
