...
def solve(schedule):
    ...
    # Initialisation aléatoire basée sur le nombre de couleur obtenue par la solution greedy
    # On conserve comme meilleure solution la solution greeedy pour l'instant pour en avoir au moins une faisable
    ...
    while time.time() < max_duration + start_time :
        ...
        if solution.max() < best_k and minima:
            best_sol = solution.copy()
            best_k = solution.max()
        ...
        # Cependant pour un choix aléatoire on choisit forcément la diversification ou on risque de boucler sur l'initialisation
