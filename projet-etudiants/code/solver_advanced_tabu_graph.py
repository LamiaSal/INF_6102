...
def solve_advanced(eternity_puzzle, tag, duration):
    ...
    inside_iter_SA = 0
    ILS_iter = 0
    score_mem.append(best_score)
    best_score_mem = [best_score]
    score_timestamp.append(0)
    ...
    while time.time() < max_duration + start_time and best_score > 0:
        ...
        score_timestamp.append(round((time.time() - start_time), 2))
        ...
        while not_stagnating:
            ...
            if time.time() > max_duration + start_time or best_score == 0:
                ...
                np.savetxt("score_mem" + str(tag) + ".txt", np.array(score_mem), delimiter=", ")
                np.savetxt("score_time" + str(tag) + ".txt", np.array(score_timestamp), delimiter=", ")
                draw_score_evolution(score_mem, score_timestamp, 83, 89)
...
def draw_score_evolution(score, score_timestamp, ls, heuristic):
    fig = plt.figure()
    ...
    y3 = [ls, ls]
    plt.plot(x3, y3, 'v--', label="Simple local")
